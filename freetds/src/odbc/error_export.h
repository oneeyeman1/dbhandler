...
static SQLRETURN odbc_SQLGetDiagRec(SQLSMALLINT handleType,
    SQLHANDLE handle,
    ...
    SQLWCHAR * szErrorMsg, SQLSMALLINT cbErrorMsgMax, SQLSMALLINT FAR* pcbErrorMsg)
{
	...
	return odbc_SQLGetDiagRec(handleType,
		handle,
		numRecord,
		(ODBC_CHAR*) szSqlStat,
		pfNativeError,
...
#ifdef ENABLE_ODBC_WIDE
static SQLRETURN odbc_SQLError(SQLHENV henv,
    SQLHDBC hdbc,
    SQLHSTMT hstmt,
    ODBC_CHAR * szSqlStat,
    SQLINTEGER * pfNativeError,
...
#ifdef ENABLE_ODBC_WIDE
static SQLRETURN odbc_SQLGetDiagField(SQLSMALLINT handleType,
    SQLHANDLE handle,
    SQLSMALLINT numRecord,
    ...
    SQLSMALLINT * pcbBuffer)
{
	TDSDUMP_LOG_FAST(TDS_DBG_FUNC, "SQLGetDiagFieldW(%d, %p, %d, %d, %p, %d, %p)\n",
			...
			(int) cbBuffer,
			pcbBuffer);
	return odbc_SQLGetDiagField(handleType,
		handle,
		numRecord,
		diagIdentifier,
		buffer,
