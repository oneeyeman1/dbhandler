...
int PostgresDatabase::GetTableListFromDb(std::vector<std::wstring> &errorMsg)
{
    ...
    std::wstring query3 = L"select x.ordinal_position AS pos, c.constraint_name AS name, x.position_in_unique_constraint AS field_pos, x.table_schema as schema, x.table_name AS table, x.column_name AS column, y.table_schema as ref_schema, y.table_name as ref_table, y.column_name as ref_column, c.update_rule, c.delete_rule from information_schema.referential_constraints c, information_schema.key_column_usage x, information_schema.key_column_usage y where x.constraint_name = c.constraint_name and y.ordinal_position = x.position_in_unique_constraint and y.constraint_name = c.unique_constraint_name AND x.table_name = 'leaguescorehitter' order by c.constraint_name, x.ordinal_position;";
