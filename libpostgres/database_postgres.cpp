...
int PostgresDatabase::CreateSystemObjectsAndGetDatabaseInfo(std::vector<std::wstring> &errorMsg)
{
    ...
    else
    {
        ...
        queries.push_back( L"DO $$ BEGIN IF NOT EXISTS( SELECT 1 FROM abcatfmt WHERE abf_name = 'h:mm AM/PM' ) THEN INSERT INTO abcatfmt VALUES( 'h:mm AM/PM', 'h:mm AM/PM', 84, 0 ) ON CONFLICT DO NOTHING;" );
        queries.push_back( L"DO $$ BEGIN IF NOT EXISTS( SELECT 1 FROM abcatfmt WHERE abf_name = 'h:mm:ss AM/PM' ) THEN INSERT INTO abcatfmt VALUES( 'h:mm:ss AM/PM', 'h:mm:ss AM/PM', 84, 0 ) ON CONFLICT DO NOTHING;" );
        queries.push_back( L"DO $$ BEGIN IF NOT EXISTS( SELECT 1 FROM abcatfmt WHERE abf_name = 'h:mm:ss' ) THEN INSERT INTO abcatfmt VALUES( 'h:mm:ss', 'h:mm:ss', 84, 0 ) ON CONFLICT DO NOTHING;" );
        queries.push_back( L"DO $$ BEGIN IF NOT EXISTS( SELECT 1 FROM abcatfmt WHERE abf_name = 'Phone_format' ) THEN INSERT INTO abcatfmt VALUES( 'Phone_format', '(@@@)) @@@-@@@@', 80, 0 ) ON CONFLICT DO NOTHING;" );
        queries.push_back( L"DO $$ BEGIN IF NOT EXISTS( SELECT 1 FROM abcatfmt WHERE abf_name = 'm-d-yy' ) THEN INSERT INTO abcatfmt VALUES( 'm-d-yy', 'm-d-yy', 84, 0 ) ON CONFLICT DO NOTHING;" );
        queries.push_back( L"DO $$ BEGIN IF NOT EXISTS( SELECT 1 FROM abcatfmt WHERE abf_name = 'soc_sec_number' ) THEN INSERT INTO abcatfmt VALUES( 'soc_sec_number', '@@@-@@-@@@@', 80, 0 ) ON CONFLICT DO NOTHING;" );
...
int PostgresDatabase::GetTableListFromDb(std::vector<std::wstring> &errorMsg)
{
    ...
    std::wstring query5 = L"SELECT abd_fhgt, abd_fwgt, abd_fitl, abd_funl, abd_fstr, abd_fchr, abd_fptc, rtrim(abd_ffce), abh_fhgt, abh_fwgt, abh_fitl, abh_funl, abh_fstr, abh_fchr, abh_fptc, rtrim(abh_ffce), abl_fhgt, abl_fwgt, abl_fitl, abl_funl, abl_fstr, abl_fchr, abl_fptc, rtrim(abl_ffce), rtrim(abt_cmnt) FROM abcattbl WHERE abt_tnam = $1 AND abt_ownr = $2 AND abt_os = $3;";
    ...
    else if( status == PGRES_TUPLES_OK )
    {
        ...
        else
        {
            for( int i = 0; i < count; i++ )
            {
                ...
                auto heading = m_pimpl->m_myconv.from_bytes( PQgetvalue( res, i, 7 ) );
                field->GetFieldProperties().m_heading.m_heading = heading == L"" ? fieldName : heading;
                field->GetFieldProperties().m_heading.m_headingAlignment = atoi( PQgetvalue( res, i, 8 ) );
                auto label = m_pimpl->m_myconv.from_bytes( PQgetvalue( res, i, 5 ) );
                field->GetFieldProperties().m_heading.m_label = label == L"" ? fieldName : label;
                field->GetFieldProperties().m_heading.m_labelAlignment = atoi( PQgetvalue( res, i, 6 ) );
