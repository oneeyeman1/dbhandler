...
int MySQLDatabase::CreateDatabase(const std::wstring &name, const std::unique_ptr<CreateDBOptions> &opts, std::vector<std::wstring> &errorMsg)
{
    int result = 0;
    result = Disconnect( errorMsg );
    std::wstring query = L"CREATE DATABASE ";
    if( opts->m_exist )
        query += L"IF NOT EXISTS ";
    query += name;
    ...
    if( !result )
    {
        ...
        {
            result = mysql_select_db( m_db, m_pimpl->m_myconv.to_bytes( name.c_str() ).c_str() );
            if( result )
            {
                result = 1;
                std::wstring err = m_pimpl->m_myconv.from_bytes( mysql_error( m_db ) );
...
int MySQLDatabase::CreateSystemObjectsAndGetDatabaseInfo(std::vector<std::wstring> &errorMsg)
{
    ...
    queries3.push_back( L"INSERT IGNORE INTO abcatedt VALUES( \'Color List\', \'Yrllow\', 87, 3, 15, 0, NULL );" );
    ...
    {
        for( std::vector<std::wstring>::iterator it = queries1.begin(); it < queries1.end(); ++it )
        {
        ...
        if( !result )
        {
            auto i = 1;
            auto create = false;
            for( std::vector<std::wstring>::iterator it1 = queries2.begin(); it1 < queries2.end(); ++it1, ++i )
            {
                if( i % 2 == 0 )
                {
                    if( create )
                    {
                        res = mysql_query( m_db, m_pimpl->m_myconv.to_bytes( (*it1).c_str() ).c_str() );
                        if( res )
                        {
                            err = m_pimpl->m_myconv.from_bytes( mysql_error( m_db ) );
                            errorMsg.push_back( err );
                ...
                else
                {
                    ...
                    resultset = mysql_store_result( m_db );
                    if( !resultset && mysql_errno( m_db ) != 0 )
                    {
                        err = m_pimpl->m_myconv.from_bytes( mysql_error( m_db ) );
                        errorMsg.push_back( err );
                        errorMsg.push_back( L"Connection to database failed!" );
                    ...
                    {
                        auto count = mysql_num_rows( resultset );
                        if( !count )
                            create = true;
                    }
...
int MySQLDatabase::GetTableListFromDb(std::vector<std::wstring> &errorMsg)
{
    ...
    std::wstring errorMessage;
    std::wstring query1 = L"SELECT t.table_catalog, t.table_schema, t.table_name, \"\" AS owner, CASE WHEN t.engine = 'InnoDB' THEN (SELECT st.table_id FROM information_schema.INNODB_TABLES st WHERE CONCAT(t.table_schema,'/', t.table_name) = st.name) ELSE (SELECT 0) END AS id FROM information_schema.tables t;";
    std::wstring query2;
    MYSQL_BIND params[6];
    MYSQL_RES *results = nullptr;
    const char *names[6] = { "OS", "fullname", "schema", "table", "table", "schema" };
    ...
    if( !result )
    {
        results = mysql_store_result( m_db );
        if( !results )
        {
            std::wstring err = m_pimpl->m_myconv.from_bytes( mysql_error( m_db ) );
            errorMsg.push_back( err );
    ...
    if( !result )
    {
        ...
        if( !result )
        {
            if( mysql_stmt_prepare( stmt, m_pimpl->m_myconv.to_bytes( query2.c_str() ).c_str(), query2.length() ) )
            {
        ...
        if( !result )
        {
            ...
            params[0].buffer = &osid;
            params[0].is_null = &is_null[0];
            ...
            while( ( row = mysql_fetch_row( results ) ) != NULL )
            {
                ...
                params[1].is_null = &is_null[1];
                params[1].length = &len[0];
                params[2].buffer_type = MYSQL_TYPE_STRING;
                params[2].buffer = schema_name;
                params[2].buffer_length = 64;
                params[2].is_null = &is_null[2];
                params[2].length = &len[1];
                params[3].buffer_type = MYSQL_TYPE_STRING;
                params[3].buffer = table_name;
                params[3].buffer_length = 64;
                params[3].is_null = &is_null[3];
                params[3].length = &len[2];
                params[4].buffer_type = MYSQL_TYPE_STRING;
                params[4].buffer = table_name;
                params[4].buffer_length = 64;
                params[4].is_null = &is_null[4];
                params[4].length = &len[2];
                params[5].buffer_type = MYSQL_TYPE_STRING;
                params[5].buffer = schema_name;
...
#else
                if( mysql_stmt_bind_named_params( stmt, params, 6, names ) )
                {
                    std::wstring err = m_pimpl->m_myconv.from_bytes( mysql_stmt_error( stmt ) );
                    errorMsg.push_back( err );
                    result = 1;
...
int MySQLDatabase::SetTableProperties(const DatabaseTable *table, const TableProperties &properties, bool isLog, std::wstring &command, std::vector<std::wstring> &errorMsg)
{
    ...
    else
    {
        ...
        std::wstring owner = table->GetTableOwner();
...
int MySQLDatabase::GetFieldProperties(const std::wstring &tableName, const std::wstring &schemaName, const std::wstring &ownerName, const std::wstring &fieldName, TableField *field, std::vector<std::wstring> &errorMsg)
{
    ...
    if( !result )
    {
        ...
        switch( res )
        {
            ...
            case MYSQL_NO_DATA:
            {
                ...
                field->GetFieldProperties().m_heading.m_label = L"";
    ...
    if( !result )
    {
        if( mysql_stmt_prepare( stmt, m_pimpl->m_myconv.to_bytes( query1.c_str() ).c_str(), query1.length() ) )
        {
            std::wstring err = m_pimpl->m_myconv.from_bytes( mysql_stmt_error( stmt ) );
            errorMsg.push_back( err );
            result = 1;
...
int MySQLDatabase::GetServerVersion(std::vector<std::wstring> &UNUSED(errorMsg))
{
    ...
    pimpl.m_versionMinor = ( ( version - pimpl.m_versionMajor ) * 10000 ) % 100;
    pimpl.m_versionRevision = version - ( pimpl.m_versionMajor * 10000 + pimpl.m_versionMinor * 100 );
    version = mysql_get_client_version();
    pimpl.m_clientVersionMajor = version / 10000;
    pimpl.m_clientVersionMinor = ( ( version - pimpl.m_clientVersionMajor ) * 10000 ) % 100;
    pimpl.m_clientVersionRevision = version - ( pimpl.m_clientVersionMajor * 10000 + pimpl.m_clientVersionMinor * 100 );
    return result;
}

int MySQLDatabase::DropForeignKey(std::wstring &command, DatabaseTable *table, const std::wstring &keyName, bool logOnly, const std::vector<FKField *> &fkFields, std::vector<std::wstring> &errorMsg)
{
    int result = 0;
...
int MySQLDatabase::AddDropTable(const std::wstring &catalog, const std::wstring &schemaName, const std::wstring &tableName, bool tableAdded, std::vector<std::wstring> &errorMsg)
{
    ...
    MYSQL_RES *meta1 = nullptr, *meta2 = nullptr, *meta3 = nullptr;
    ...
    std::vector<std::wstring> fk_names, indexes, pk;
    ...
    if( tableAdded )
    {
        ...
        memset( params, 0, sizeof( params ) );
        len[0] = strlen( m_pimpl->m_myconv.to_bytes( catalog.c_str() ).c_str() ) + 1;
        len[1] = strlen( m_pimpl->m_myconv.to_bytes( schemaName.c_str() ).c_str() ) + 1;
        len[2] = strlen( m_pimpl->m_myconv.to_bytes( tableName.c_str() ).c_str() ) + 1;
        ...
        if( !result )
        {
            ...
            results1[0].buffer = &constraintName;
...
//            id, name, orig_schema, table_name, original_field, ref_schema, ref_table, referenced_field, origFields, refFields, update_constraint, delete_constraint, match = -1
                foreign_keys[ordinal].push_back( new FKField( ordinal, m_pimpl->m_myconv.from_bytes( constraintName ), schemaName, tableName, m_pimpl->m_myconv.from_bytes( fieldName ),
        ...
        if( !result )
        {
            table = new DatabaseTable( tableName, schemaName, fields, pk, foreign_keys );
            table->SetFullName( catalog + L"." + schemaName + L"." + tableName );
            table->SetCatalog( catalog );
            table->SetNumberOfFields( numFields );
            table->SetTableOwner( L"" );
        ...
        if( !result )
        {
            ...
            {
                indexes.push_back( m_pimpl->m_myconv.from_bytes( fieldName ) );
                numIndexes++;
            }
...
int MySQLDatabase::GetCreateDBOptions(std::unique_ptr<CreateDBOptions> &options, std::vector<std::wstring> &errorMsg)
{
    int result = 0;
    options.reset( new MySQLCreateDBOptions( L"", L"", L"", false , false ) );
    ...
    if( !result )
    {
        ...
        {
            dynamic_cast<MySQLCreateDBOptions *>( options.get() )->m_charSets.emplace_back( new CharSet( std::make_tuple( m_pimpl->m_myconv.from_bytes( row[0] ), m_pimpl->m_myconv.from_bytes( row[2] ), m_pimpl->m_myconv.from_bytes( row[1] )  ) ) );
        }
    ...
    if( !result )
    {
        ...
        while( ( row = mysql_fetch_row( res ) ) )
        {
            ...
            if( m_pimpl->m_myconv.from_bytes( row[4] ) != L"Y" )
                comp = false;
            dynamic_cast<MySQLCreateDBOptions *>( options.get() )->m_collations[m_pimpl->m_myconv.from_bytes( row[1] )].push_back( std::make_tuple( m_pimpl->m_myconv.from_bytes( row[0] ), def, comp ) );
        }
        mysql_free_result( res );
    }
    return result;
