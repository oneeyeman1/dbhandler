                                                                                        ...
                                                                                        else
                                                                                        {
                                                                                            ...
                                                                                            else
                                                                                            {
                                                                                                ...
                                                                                                else
                                                                                                {
                                                                                                    ...
                                                                                                    results3[0].is_null = (my_bool*) &is_null3[0];
                                                                                                    results3[0].length = &length[0];
                                                                                                    results3[0].error = (my_bool*) &error3[0];
...
int MySQLDatabase::TokenizeConnectionString(const std::wstring &connectStr, std::vector<std::wstring> &errorMsg)
{
    ...
    while( !tmp.empty() )
    {
        ...
        else
        {
            ...
            if( option >= MYSQL_OPT_CONNECT_TIMEOUT && option < MYSQL_PROGRESS_CALLBACK )
	      /* All enum mysql_option values before mariadb */
            {
            ...
            if( !mysql_stmt_bind_param( res, bind ) )
            {
                ...
                if( !mysql_stmt_execute( res ) )
                {
                    if( ( mysql_store_result( m_db ) ) )
                    {
                        ...
                        results.is_null = (my_bool*) &is_null;
                        results.length = &length;
                        results.error = (my_bool*) &error;
