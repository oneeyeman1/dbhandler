std::wstring createtable = L"DECLARE @table_name SYSNAME" \
"SELECT @table_name = 'dbo.leagues'" \
"DECLARE " \
"      @object_name SYSNAME " \
"    , @object_id INT " \
"SELECT " \
...
"    WHERE ic.[object_id] = c.[object_id] AND ic.column_id = " \
"c.column_id AND ic.[object_id] = @object_id " \
"), " \
"fk_columns AS " \
"(" \
"     SELECT " \
...
"        , cname = c.name " \
"        , rcname = rc.name " \
"    FROM sys.foreign_key_columns k, sys.columns rc, sys.columns c " \
"    WHERE rc.[object_id] = k.referenced_object_id AND rc.column_id = " \
"k.referenced_column_id " \
"          AND c.[object_id] = k.parent_object_id AND c.column_id = " \
"k.parent_column_id " \
...
"'binary', 'text')" \
"                       THEN '(' + CASE WHEN c.max_length = -1 THEN " \
"'MAX' ELSE CAST(c.max_length AS VARCHAR(5)) END + ')' "\
"                     WHEN tp.name IN ('nvarchar', 'nchar', 'ntext') " \
"                       THEN '(' + CASE WHEN c.max_length = -1 THEN " \
"'MAX' ELSE CAST(c.max_length / 2 AS VARCHAR(5)) END + ')' " \
...
"    LEFT JOIN sys.computed_columns cc ON c.[object_id] = " \
"cc.[object_id] AND c.column_id = cc.column_id " \
"    LEFT JOIN sys.default_constraints dc ON c.default_object_id != 0 " \
"AND c.[object_id] = dc.parent_object_id AND c.column_id = " \
"dc.parent_column_id " \
"    LEFT JOIN sys.identity_columns ic ON c.is_identity = 1 AND " \
...
"                             AND c.[object_id] = ic.[object_id] AND " \
"c.column_id = ic.column_id " \
"                         FOR XML PATH(N''), TYPE).value('.', " \
"'NVARCHAR(MAX)'), 1, 2, '')) " \
"            + ')' + " \
"            (SELECT CASE WHEN ind.type_desc = 'UNCLUSTERED' THEN ' " \
"UNCLUSTERED' ELSE ' CLUSTERED' END " \
...
"        FROM sys.foreign_keys fk, sys.objects ro " \
"        WHERE fk.parent_object_id = @object_id " \
"              AND ro.[object_id] = fk.referenced_object_id " \
"        FOR XML PATH(N''), TYPE).value('.', 'NVARCHAR(MAX)')), '') " \
"    + ISNULL(((SELECT " \
"         CHAR(13) + 'CREATE' + CASE WHEN i.is_unique = 1 THEN ' " \
"UNIQUE' ELSE '' END " \
...
"'NVARCHAR(MAX)'), 1, 2, '') + ')', '') + CASE WHEN " \
"ISNULL(i.filter_definition,'') = '' THEN '' ELSE ' WHERE ' + " \
"i.filter_definition END + CHAR(13) " \
"        FROM sys.indexes i " \
