// -*- C++ -*-
//
// generated by wxGlade 0.7.2 (standalone edition) on Sun Oct 30 13:18:41 2016
//
// Example for compiling a single file project under Linux using g++:
//  g++ MyApp.cpp $(wx-config --libs) $(wx-config --cxxflags) -o MyApp
//
// Example for compiling a multi file project under Linux using g++:
//  g++ main.cpp $(wx-config --libs) $(wx-config --cxxflags) -o MyApp Dialog1.cpp Frame1.cpp
//

#include "createindex.h"

// begin wxGlade: ::extracode
// end wxGlade



CreateIndex::CreateIndex(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style):
    wxDialog(parent, id, title, pos, size, style)
{
    // begin wxGlade: CreateIndex::CreateIndex
    panel_1 = new wxPanel(this, wxID_ANY);
    m_label1 = new wxStaticText(panel_1, wxID_ANY, _("Table"));
    m_tableName = new wxStaticText(panel_1, wxID_ANY, wxEmptyString);
    m_label2 = new wxStaticText(panel_1, wxID_ANY, _("&Index Name"));
    m_indexName = new wxTextCtrl(panel_1, wxID_ANY, wxEmptyString);
    m_unique = new wxRadioButton(panel_1, wxID_ANY, _("&Unique"), wxDefaultPosition, wxDefaultSize, wxRB_GROUP);
    m_duplicate = new wxRadioButton(panel_1, wxID_ANY, _("&Duplicate"));
    m_ascending = new wxRadioButton(panel_1, wxID_ANY, _("&Ascending"), wxDefaultPosition, wxDefaultSize, wxRB_GROUP);
    m_descending = new wxRadioButton(panel_1, wxID_ANY, _("&Descending"));
    m_label3 = new wxStaticText(panel_1, wxID_ANY, _("Index Columns:"));
    m_indexColumns = new wxTextCtrl(panel_1, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxHSCROLL|wxTE_MULTILINE);
    const wxString m_table_choices[] = {
        _("choice 1"),
    };
    m_table = new wxListBox(panel_1, wxID_ANY, wxDefaultPosition, wxDefaultSize, 1, m_table_choices, wxLB_MULTIPLE);
    m_OK = new wxButton(panel_1, wxID_OK, _("OK"));
    m_logOnly = new wxButton(panel_1, wxID_ANY, _("&Log Only"));
    m_cancel = new wxButton(panel_1, wxID_CANCEL, _("Cancel"));
    m_help = new wxButton(panel_1, wxID_HELP, _("&Help"));

    set_properties();
    do_layout();
    // end wxGlade
}


void CreateIndex::set_properties()
{
    // begin wxGlade: CreateIndex::set_properties
    SetTitle(_("Create Index"));
    m_table->SetSelection(0);
    m_OK->SetDefault();
    // end wxGlade
}


void CreateIndex::do_layout()
{
    // begin wxGlade: CreateIndex::do_layout
    wxBoxSizer* sizer_2 = new wxBoxSizer(wxHORIZONTAL);
    wxBoxSizer* sizer_3 = new wxBoxSizer(wxHORIZONTAL);
    wxBoxSizer* sizer_4 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_5 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_14 = new wxBoxSizer(wxHORIZONTAL);
    wxBoxSizer* sizer_15 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_13 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_6 = new wxBoxSizer(wxHORIZONTAL);
    wxBoxSizer* sizer_10 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_12 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_11 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_7 = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* sizer_9 = new wxBoxSizer(wxHORIZONTAL);
    wxBoxSizer* sizer_8 = new wxBoxSizer(wxHORIZONTAL);
    sizer_3->Add(10, 10, 0, wxEXPAND, 0);
    sizer_4->Add(10, 10, 0, wxEXPAND, 0);
    sizer_8->Add(m_label1, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_8->Add(10, 10, 0, wxEXPAND, 0);
    sizer_8->Add(m_tableName, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_7->Add(sizer_8, 1, 0, 0);
    sizer_7->Add(5, 5, 0, wxEXPAND, 0);
    sizer_9->Add(m_label2, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_9->Add(10, 10, 0, wxEXPAND, 0);
    sizer_9->Add(m_indexName, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_7->Add(sizer_9, 1, 0, 0);
    sizer_6->Add(sizer_7, 1, 0, 0);
    sizer_6->Add(20, 20, 0, wxEXPAND, 0);
    sizer_11->Add(m_unique, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_11->Add(m_duplicate, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_10->Add(sizer_11, 1, 0, 0);
    sizer_10->Add(5, 5, 0, wxEXPAND, 0);
    sizer_12->Add(m_ascending, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_12->Add(m_descending, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_10->Add(sizer_12, 1, 0, 0);
    sizer_6->Add(sizer_10, 1, 0, 0);
    sizer_5->Add(sizer_6, 1, 0, 0);
    sizer_5->Add(30, 30, 0, wxEXPAND, 0);
    sizer_13->Add(m_label3, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_13->Add(5, 5, 0, wxEXPAND, 0);
    sizer_13->Add(m_indexColumns, 0, wxALIGN_CENTER_VERTICAL|wxEXPAND, 0);
    sizer_5->Add(sizer_13, 1, 0, 0);
    sizer_5->Add(5, 5, 0, wxEXPAND, 0);
    sizer_14->Add(m_table, 0, wxALIGN_BOTTOM, 0);
    sizer_14->Add(30, 30, 0, wxEXPAND, 0);
    sizer_15->Add(m_OK, 0, wxALIGN_CENTER, 0);
    sizer_15->Add(m_logOnly, 0, wxALIGN_CENTER, 0);
    sizer_15->Add(m_cancel, 0, wxALIGN_CENTER, 0);
    sizer_15->Add(m_help, 0, wxALIGN_CENTER, 0);
    sizer_14->Add(sizer_15, 1, 0, 0);
    sizer_5->Add(sizer_14, 1, 0, 0);
    sizer_4->Add(sizer_5, 1, 0, 0);
    sizer_4->Add(10, 10, 0, wxEXPAND, 0);
    sizer_3->Add(sizer_4, 1, 0, 0);
    sizer_3->Add(10, 10, 0, wxEXPAND, 0);
    panel_1->SetSizer(sizer_3);
    sizer_2->Add(panel_1, 1, 0, 0);
    SetSizer(sizer_2);
    sizer_2->Fit(this);
    Layout();
    // end wxGlade
}

