...
extern "C" WXEXPORT int SelectTablesForView(wxWindow *parent, Database *db, std::map<wxString, std::vector<TableDefinition> >&tableNames, std::vector<std::wstring> &names, bool isTableView, const int type, bool isNewView, std::vector<std::wstring> &errors)
{
    int res;
    ;
#ifdef __WXMSW__
    ...
    if( res != wxID_CANCEL )
    {
        ...
        for( auto cat : tableNames )
        {
            ...
            {
                if( db->AddDropTable( cat.first.ToStdWstring(), def.schemaName, def.tableName, true, errors ) )
                    break;
...
extern "C" WXEXPORT int AddEditMask(wxWindow *parent, bool isNew, const wxString &type, wxString &name, wxString &format, const FieldTableDisplayProperties &prop, Database *db)
{
#ifdef __WXMSW__
    wxTheApp->SetTopWindow( parent );
#endif
    ...
    if( ret == wxID_OK )
    {
        name = dlg.GetMaskNameCtrl()->GetValue();
        format = dlg.GetFormatCtrl()->GetValue();
    }
    return ret;
}
...
extern  "C" WXEXPORT int CreateDB(wxWindow *parent, const std::wstring &type, const std::wstring &subtype, int versionMajor, int versionMinor, std::unique_ptr<CreateDBOptions> &options)
{
    int res;
#ifdef __WXMSW__
    wxTheApp->SetTopWindow( parent );
