...
extern "C" WXEXPORT int ChooseObject(wxWindow *parent, int objectId, const std::vector<QueryInfo> &queries, wxString &docuentName)
{
    int res;
    wxString title;
#ifdef __WXMSW__
    ...
    res = dlg.ShowModal();
    if( res == wxID_OK )
        docuentName = dlg.GetDocumentName()->GetValue();
    return res;
}
