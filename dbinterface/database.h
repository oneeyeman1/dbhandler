...
struct PostgresCreateDBOptions : public CreateDBOptions
{
    ...
    int m_connlimit;
    bool m_allowConn = true, m_isTemplate = false;
...
struct PostgresPKOptions : public PKOptions
{
    ...
    bool m_withoutOverlaps, m_cascade;
    PostgresPKOptions(const std::wstring &name, const std::wstring &type, const std::wstring &columns, const std::wstring &storage, const std::wstring &tablespace, bool overlaps, bool cascade) : m_includeColumns( columns ), m_type( type ), m_storage( storage ), m_tablespace( tablespace ), m_withoutOverlaps( overlaps ), m_cascade( cascade ) { m_name = name; }
...
struct MySQLPKOptions : public PKOptions
{
    ...
    virtual ~MySQLPKOptions() {};
    virtual bool equal(const PKOptions &opt) const override
    {
        const MySQLPKOptions *options = dynamic_cast<const MySQLPKOptions *>( &opt );
        return m_indexType == options->m_indexType && m_parser == options->m_parser &&
               m_comment == options->m_comment && m_attr1 == options->m_attr1 &&
               m_attr2 == options->m_attr2 && m_blocksize == options->m_blocksize &&
               m_isVisible == options->m_isVisible;
    }
    virtual bool notequal(const PKOptions &opt) const override
    {
        const MySQLPKOptions *options = dynamic_cast<const MySQLPKOptions*>( &opt );
...
struct FieldTableValidationProperties
{
    ...
    std::wstring m_validator, m_fieldName;
...
public:
    ...
    virtual int CreateDatabase(const std::wstring &name, const std::unique_ptr<CreateDBOptions> &opts, std::vector<std::wstring> &errorMsg) = 0;
    ...
    virtual int DropIndex(const std::wstring &fullTableName, const std::wstring &indexName, const DropIndexOption &options, std::vector<std::wstring> &errorMsg) = 0;
    ...
    virtual int GetTableCreationSyntax(const std::wstring &catalog, const std::wstring &schema, const std::wstring &table, std::vector<std::wstring> &syntax, std::vector<std::wstring> &errorMsg) = 0;
    ...
    virtual int GetCreateDBOptions(std::unique_ptr<CreateDBOptions> &options, std::vector<std::wstring> &errors) = 0;
};
